...
SSH_COMMIT_SIGNING="${SSHCOMMITSIGNING:-true}"
...
# esc <value> — make a value safe inside the double-quoted assignments below.
#
# config.env is *sourced* by the lifecycle scripts, so an option value is shell
# input, not data. Written raw, a value containing `$(`, a backtick or `$VAR` is
# evaluated at create time as the remote user, and one containing a quote makes
# config.env unparseable — which under post-create's `set -euo pipefail` aborts it
# at the `. config.env` line, before a single step has run. That is the one
# failure mode this Feature must not have: a container that refuses to come up is
# not fixable from a terminal, which is the whole reason every step in setup.sh
# skips rather than exits.
#
# Escaping the four characters that are still special inside double quotes keeps
# the `KEY="value"` shape the file has always had, so anything grepping it — the
# test suite included — keeps working.
esc() {
...
DEVBASE_GH_CREDENTIAL_HELPER="$(esc "${GH_CREDENTIAL_HELPER}")"
DEVBASE_SSH_COMMIT_SIGNING="$(esc "${SSH_COMMIT_SIGNING}")"
DEVBASE_ENV_INFO_BANNER="$(esc "${ENV_INFO_BANNER}")"
DEVBASE_INSTALL_PROJECT_DEPENDENCIES="$(esc "${INSTALL_PROJECT_DEPENDENCIES}")"
DEVBASE_AUTOLOAD_ENV_SCRIPT="$(esc "${AUTOLOAD_ENV_SCRIPT}")"
DEVBASE_INSTALL_RTK="$(esc "${INSTALL_RTK}")"
DEVBASE_RTK_VERSION="$(esc "${RTK_VERSION}")"
EOF
chmod 0644 "${SHARE_DIR}/config.env"

# A config.env that will not source takes the container down with it, so this is
# verified rather than assumed. `sh -n` parses without executing.
if ! sh -n "${SHARE_DIR}/config.env" 2>/dev/null; then
    echo "devbase: ERROR: generated ${SHARE_DIR}/config.env is not valid shell" >&2
    exit 1
fi
